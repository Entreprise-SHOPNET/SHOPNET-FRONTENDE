...
// CONFIGURATION
// ============================================
const API_URL = "https://shopnet-backend.onrender.com/api/products/ai/beauty";
      ...
      <View style={[styles.imageContainer, { backgroundColor: colors.imageBg }]}>
        ...
        {showCategoryBadge && (
          <View style={[styles.categoryBadge, { backgroundColor: colors.accent }]}>
            <Text style={styles.categoryBadgeText}>💄 Beauté</Text>
          </View>
        )}
        {discount > 0 && (
...
// ============================================
// ÉCRAN PRINCIPAL BEAUTY AI
// ============================================
export default function BeautyAIScreen() {
  ...
  const fetchProducts = async (pageNum: number, shouldCache = true) => {
    try {
      const cacheKey = `beauty_ai_${pageNum}`;
      if (shouldCache) {
      ...
      console.log("🔍 Fetching beauty AI:", url);
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await AsyncStorage.removeItem("beauty_ai_1");
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={[styles.emptyText, { color: COLORS.emptyIcon }]}>
          {error || "Aucun produit beauté trouvé"}
        </Text>
  ...
  if (loading && rows.length === 0) {
    return (
      <SafeAreaView style={[styles.loadingContainer, { backgroundColor: COLORS.loadingBg }]}>
        ...
        <Text style={[styles.loadingText, { color: COLORS.textSecondary }]}>Chargement des produits beauté...</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: COLORS.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: COLORS.headerBg, borderBottomColor: COLORS.headerBorder }]}>
        ...
        <Text style={[styles.headerTitle, { color: COLORS.text }]}>💄 Beauté IA</Text>
