...
// CONFIGURATION
// ============================================
const API_URL = "https://shopnet-backend.onrender.com/api/products/electronics/advanced";
...
// Génère des lignes avec motif 3‑2‑4‑2 (style Temu/AliExpress)
...
const ProductCard = ({ 
  item, 
  onPress, 
  showCategoryBadge = true  // ← nouveau prop
}: { 
  item: Product; 
  onPress: () => void;
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.7}>
      <View style={styles.imageContainer}>
        ...
        />
        {/* Badge catégorie Électronique - conditionnel */}
        {showCategoryBadge && (
          <View style={styles.categoryBadge}>
            <Text style={styles.categoryBadgeText}>📱 Électronique</Text>
          </View>
        )}
        {discount > 0 && (
...
// ÉCRAN PRINCIPAL
// ============================================
export default function ElectronicsAdvancedScreen() {
  ...
  const fetchProducts = async (pageNum: number, shouldCache = true) => {
    try {
      const cacheKey = `electronics_advanced_${pageNum}`;
      if (shouldCache) {
  ...
  // Chargement de la page suivante (sans mélange)
  ...
  // Rafraîchissement : invalide le cache, recharge la première page et mélange
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await AsyncStorage.removeItem("electronics_advanced_1");
  ...
  const renderRow = ({ item }: { item: Row }) => {
    // Pour les lignes de 2 produits (cartes plus grandes) → afficher le badge
    // Pour les lignes de 3 ou 4 produits (petites cartes) → masquer le badge
  ...
  const renderEmpty = () => {
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyText}>{error || "Aucun produit électronique"}</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>📱 Électronique</Text>
...
// ============================================
// STYLES (sans ombres, uniquement bordures fines)
// ============================================
