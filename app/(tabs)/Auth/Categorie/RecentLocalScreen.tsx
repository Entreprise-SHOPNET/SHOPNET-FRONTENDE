...
// CONFIGURATION
// ============================================
const API_URL = "https://shopnet-backend.onrender.com/api/products/ai/recent-local";
const CACHE_TTL = 2 * 60 * 1000; // 2 minutes (car contenu très frais)
...
const isLessThan6Hours = (createdAt: string): boolean => {
  const now = new Date();
...
const ProductCard = ({
  ...
  const isUltraRecent = isLessThan6Hours(item.created_at);
      ...
      <View style={[styles.imageContainer, { backgroundColor: colors.imageBg }]}>
        ...
        {showCategoryBadge && (
          <View style={[styles.categoryBadge, { backgroundColor: colors.accent }]}>
            <Text style={styles.categoryBadgeText}>🕐 -24h</Text>
          </View>
        )}
        {discount > 0 && (
        ...
        {isUltraRecent && (
          <View style={[styles.clockBadge, { backgroundColor: colors.clockBg }]}>
            ...
            <Text style={[styles.clockText, { color: colors.clockText }]}>Flash</Text>
          </View>
        )}
      </View>
        ...
        <View style={styles.footerRow}>
          ...
          {item.distance && (
            <View style={[styles.distanceBadge, { backgroundColor: colors.distanceBg }]}>
              ...
              <Text style={[styles.distanceText, { color: colors.textSecondary }]}>{formatDistance(item.distance)}</Text>
            </View>
          )}
        </View>
      </View>
...
// ============================================
// ÉCRAN PRINCIPAL RECENT LOCAL
// ============================================
export default function RecentLocalScreen() {
  ...
  const fetchProducts = async (pageNum: number, shouldCache = true) => {
    try {
      const cacheKey = `recent_local_${pageNum}`;
      if (shouldCache) {
      ...
      console.log("🔍 Fetching recent local:", url);
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await AsyncStorage.removeItem("recent_local_1");
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={[styles.emptyText, { color: COLORS.emptyIcon }]}>
          {error || "Aucun produit récent trouvé"}
        </Text>
  ...
  if (loading && rows.length === 0) {
    return (
      <SafeAreaView style={[styles.loadingContainer, { backgroundColor: COLORS.loadingBg }]}>
        ...
        <Text style={[styles.loadingText, { color: COLORS.textSecondary }]}>Chargement des nouveautés...</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: COLORS.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: COLORS.headerBg, borderBottomColor: COLORS.headerBorder }]}>
        ...
        <Text style={[styles.headerTitle, { color: COLORS.text }]}>🕐 Nouveautés -24h</Text>
